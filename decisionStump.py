# Author:      Lane Moseley
# Description: This file demonstrates the usage of the custom decision stump
...
from ML import DecisionStump, plot_decision_regions
import numpy as np
import pandas as pd
...
def main():
    ...
    boost = AdaBoostClassifier(base_estimator=DecisionStump(),
                               algorithm='SAMME', n_estimators=10, learning_rate=1.0)
    boost.fit(X, Y)

    # plot the decision regions and display metrics to the console
    plot_decision_regions(X, Y, boost, 0.01, x_label=xlabel, y_label=ylabel,
                          title=title + "\nML.py Decision Stump Classifier")
    print("scikit-learn AdaBoost With ML.py Decision Stump Classifier")
    print("Score:", boost.score(X, Y))
    ####################################################################################################################
